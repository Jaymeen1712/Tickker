"use client";
import { fetchWeeklyRevenueByProfileId } from "@/db/queries";
import { useAppStore } from "@/store";
import { handleAPIResponse } from "@/utils";
import { startOfWeek, subWeeks } from "date-fns";
import { useCallback, useEffect, useState } from "react";

const useRevenueGraphController = () => {
  ...
  const handleGetGraphData = useCallback(async () => {
    ...
    try {
      ...
      const { response, errors } = await fetchWeeklyRevenueByProfileId({
        profileId,
...
export default useRevenueGraphController;
